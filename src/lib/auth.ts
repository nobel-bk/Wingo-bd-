...
export async function createUser(username: string, plan: User["plan"]): Promise<User> {
  const now = Date.now();
...
export async function login(input: string): Promise<LoginResult> {
  ...
  // ── Admin check (instant, no network needed)
  if (checkAdminPassword(code)) {
    const session: Session = { type: "admin" };
    setSession(session);
    return { ok: true, role: "admin", session };
  }

  // ── Legacy long-token check
  if (code.length > 12) {
    const decoded = decodeCode(code);
    if (decoded) {
      if (Date.now() > decoded.expiresAt) return { ok: false, reason: "expired" };
  ...
  // ── Short code: try cloud first, fall back to local cache
  let users = await fetchUsersFromCloud();

  // If cloud returned nothing but we have a local cache, use it
  if (users.length === 0) {
    users = getUsers();
  }
  ...
  // Update sessionToken in background (non-blocking — don't await)
  user.sessionToken = sessionToken;
  saveUsersLocal(users.map(u2 => u2.id === user.id ? user : u2));
  writeKV(`wingobd_user_data_${user.id}`, JSON.stringify(user)).catch(() => {});
