...
export function AdminPanel({ onLogout }: { onLogout: () => void }) {
  ...
  const handleCreate = async () => {
    ...
    const user = await createUser(newUsername.trim(), newPlan);
    setNewUsername("");
    refresh();
    addLog(`➕ Created license for "${user.username}" — ${user.plan} plan`);
  };

  const handleCopy = (code: string, username: string) => {
    navigator.clipboard.writeText(code).catch(() => {});
    setCopied(code);
    setTimeout(() => setCopied(null), 2000);
    addLog(`📋 Copied code for "${username}"`);
  };
            ...
            <section className="glass-panel fade-up" style={{ animationDelay: "0.12s" }}>
              ...
              <div style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap" }}>
                <div style={{ flex: 1, minWidth: 200 }}>
                  <p style={{ fontSize: 10.5, fontWeight: 700, color: "var(--muted-foreground)", marginBottom: 5, textTransform: "uppercase" }}>Client Name / ID</p>
                  <input
                    className="premium-input" value={newUsername}
                    onChange={e => setNewUsername(e.target.value)}
                    placeholder="Enter client name or Telegram ID…"
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "0.75rem" }}>
                          ...
                          <div>
                            ...
                            <div style={{ marginTop: 8, display: "inline-flex", alignItems: "center", gap: 8, background: "var(--primary-light)", borderRadius: 8, padding: "4px 10px", border: "1px solid rgba(91,94,244,0.15)" }}>
                              <code className="mono-font" style={{ fontSize: 13, color: "var(--primary)", fontWeight: 700, letterSpacing: "0.12em" }}>{u.code}</code>
                            </div>
                          </div>
                          ...
                          <div style={{ display: "flex", gap: "0.35rem", flexWrap: "wrap" }}>
                            ...
                            <button onClick={() => handleCopy(u.code, u.username)} className="pill-button"
                              style={{ borderRadius: 8, padding: "0.4rem 0.8rem", fontSize: 11,
                                color: copied === u.code ? "var(--green)" : "var(--primary)" }}>
